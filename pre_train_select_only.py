...
def train(epoch):         
    ...
    for batch_idx,sample in enumerate(train_data.get_loader()):           
        ...
        theta=model_select(sample['label'],sample['size'])
...
def test():    
    ...
    if (test_loss<bestloss):
        ...
        torch.save(model_select,"./preBestNet_select_only")
def printoutput():
    ...
    for sample in test_data.get_loader():
        ...
        input("check")
        '''
        cens=torch.floor(calc_centroid(sample['label_org']))
        '''
        for i in range(sample['image'].size()[0]):    
            for j in range(9):
                cens[i,j,0]=cens[i,j,0]*sample['size'][0][i]/128.0
        ...
        for i in range(sample['image'].size()[0]):             
            ...
            output2=sample['label'][i].cpu().clone();
            ...
            for j in range(6):   
                ...
                image3=unloader(np.uint8(image3[0].squeeze(0).cpu().detach()));
...
if Training:
    for epoch in range(epoch_num):
        ...
        test()     
    torch.save(model_select,"./preNetdata_select_only")   
    '''
