...
train_data=data_loader_Aug("train",batch_size,"stage2");
test_data=data_loader_Aug("test",batch_size,"stage2");
val_data=data_loader_Aug("val",batch_size,"stage2");
...
def train(epoch):   
    ...
    for batch_idx,sample in enumerate(train_data.get_loader()):   
        ...
        for i in range(6):                 
            ...
            loss_tmp=fun.cross_entropy(stage2_label[i],parts_label2[i].argmax(dim=1, keepdim=False))                        
    ...
    for sample in test_data.get_loader():
        ...
        for i in range(6):            
            ...
            for j in range(sample['image'].shape[0]):
                path=pre_stage2_output_path+'/'+test_data.get_namelist()[k+j]; 
                if not os.path.exists(path):
                    os.makedirs(path); 
                image=TF.to_pil_image(parts_label[i][j][1].unsqueeze(0).cpu());
